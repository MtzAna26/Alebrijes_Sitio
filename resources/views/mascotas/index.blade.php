...
<body>
    ...
    <div class="container mx-auto p-4">
        <h1 class="text-3xl font-bold mb-4">Listado de Mascotas.🐱🐶</h1>
        @if(session('mensaje'))
            <div class="alert alert-success">
                {{ session('mensaje') }}
            </div>
        @endif
        <a href="{{ route('mascotas.create') }}" class="btn btn-primary mb-4" onclick="return confirm('¿Estás seguro de agregar una nueva mascota?')">Agregar Nueva Mascota</a>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($mascotas as $mascota)
                <div class="bg-white p-4 rounded-lg shadow">
                    <h2 class="text-lg font-semibold mb-2">{{ $mascota->nombre_mascota }}</h2>
                    <p>ID: {{ $mascota->id }}</p>
                    <p>Fecha de Consulta: {{ $mascota->fecha_consulta }}</p>
                    <p>Nombre del Propietario: {{ $mascota->nombre_propietario }}</p>
                    <p>Especie: {{ $mascota->especie }}</p>
                    <p>Sexo: {{ $mascota->sexo }}</p>
                    <p>Raza: {{ $mascota->raza }}</p>
                    <p>Señas particulares de la mascota:{{ $mascota->senas_particulares }}</p>
                    <p>Alergias: {{ $mascota->alergias }}</p>
                    <p>¿Tú mascota se encuentra esterilizada? {{ $mascota->esterilizado ? 'Sí' : 'No' }}</p>
                    <p>Enfermedades previas:{{ $mascota->enfermedades_previas }}</p>
                    <p>Notas especiales: {{ $mascota->notas }}</p>

                    <div class="mt-3">
                        <a href="{{ route('mascotas.show', $mascota->id) }}" class="btn btn-info">Ver</a>
                        <a href="{{ route('mascotas.edit', $mascota->id) }}" class="btn btn-warning" onclick="return confirm('¿Estás seguro de editar esta mascota?')">Editar</a>
