...
<body>
    ...
    <div class="container mx-auto p-4">
        <h1 class="text-3xl font-bold mb-4">Listado de Propietarios.👩‍⚕️</h1>
        @if(session('mensaje'))
            <div class="alert alert-success">
                {{ session('mensaje') }}
            </div>
        @endif
        <a href="{{ route('propietario.create') }}" class="btn btn-primary mb-4" onclick="return confirm('¿Estás seguro de agregar un nuevo propietario?')">Agregar Nuevo Propietario</a>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($propietarios as $propietario)
                <div class="bg-white p-4 rounded-lg shadow">
                    <h2 class="text-lg font-semibold mb-2">{{ $propietario->nombre_propietario }}</h2>
                    <p>ID: {{ $propietario->id }}</p>
                    <p>Fecha de Consulta: {{ $propietario->fecha_consulta }}</p>
                    <p>Nombre de la Mascota: {{ $propietario->nombre_mascota }}</p>
                    <p>Dirección: {{ $propietario->direccion }}</p>
                    <p>Teléfono: {{ $propietario->telefono }}</p>

                    <div class="mt-3">
                        <a href="{{ route('propietario.show', $propietario->id) }}" class="btn btn-info">Ver</a>
                        <a href="{{ route('propietario.edit', $propietario->id) }}" class="btn btn-warning" onclick="return confirm('¿Estás seguro de editar este propietario?')">Editar</a>
                        <form action="{{ route('propietario.destroy', $propietario->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" onclick="return confirm('¿Estás seguro de eliminar este propietario?')">Eliminar</button>
                        </form>
                    </div>
                </div>
            @endforeach
